...
TesteUser(userTyped: userTyped, passwordTyped: passwordTyped)
...
userTyped = "User123@"
passwordTyped = "123ICantChange"
print(TesteUser(userTyped: userTyped, passwordTyped: passwordTyped))

// Refatorando através da criação de uma função
func TesteUser(userTyped: String, passwordTyped: String) -> String{
    if userTyped == generalUser && passwordTyped == generalPassword {
        return "Olá, \(userTyped)! A empresa lhe dá as boas vindas!"
    }else{
        return "Houve um problema no seu login. Verifique os dados digitados, por favor."
    }
}
