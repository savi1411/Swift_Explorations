...
let calcAveraging = (firstNote + secondNote) / 2

if (calcAveraging > 6.0 && numStudentAbsences < 26) {
    print("Approved student with media \(calcAveraging)")
    print("PTBR Translation: Aluno aprovado com média \(calcAveraging)")
} else {
    print("Failed student with media \(calcAveraging)")
    print("PTBR Translation: Aluno reprovado com média \(calcAveraging)")
}
