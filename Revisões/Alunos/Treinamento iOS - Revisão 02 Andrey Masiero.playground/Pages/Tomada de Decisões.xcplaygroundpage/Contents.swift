...
let phrase = ["acorde","teste","bolinho","alarme"]
let possuiu = ["acorde","bolinho"]
//
for index in phrase {
    for item in possuiu {
        if index.contains(item) {
            print("A  frase  contém a palavra \(item)")
        }
    }
}
...
for index in phrase {
    for item in possuiu {
        switch index.contains(item) {
        case true:
            print("Palavra \(index) foi encontrada")
        default:
            break
        }
