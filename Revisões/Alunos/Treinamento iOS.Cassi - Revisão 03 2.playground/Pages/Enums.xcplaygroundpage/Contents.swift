...
print("Seu signo é: \(sign)", terminator:" - ") /// terminator serve pra o print nao saltar linha
switch sign {
...
enum Month {
    ...
    var month: String {
        switch self {
        case .dezembro:
            if day < 22 { sign = "♐️ - Sagitário" } else { sign = "♑️ - Capricórnio" }
        case .janeiro:
            if day < 20 { sign = "♑️ - Capricórnio" } else { sign = "♒️ - Aquário" }
        case .fevereiro:
            if day < 19 { sign = "♒️ - Aquário" } else { sign = "♓️ - Peixes" }
        case .março:
            if day < 21 { sign = "♓️ - Peixes" } else { sign = "♈️ - Áries" }
        case .abril:
            if day < 20 { sign = "♈️ - Áries" } else { sign = "♉️ - Touro" }
        case .maio:
            if day < 21 { sign = "♉️ - Touro " } else { sign = "♊️ - Gêmeos" }
        case .junho:
            if day < 21 { sign = "♊️ - Gêmeos" } else { sign = "♋️ - Câncer" }
        case .julho:
            if day < 23 { sign = "♋️ - Câncer" } else { sign = "♌️ Leão" }
        case .agosto:
            if day < 23 { sign = "♌️ - Leão" } else { sign = "♍️ - Virgem" }
        case .setembro:
            if day < 23 { sign = "♍️ - Virgem" } else { sign = "♎️ - Libra" }
        case .outubro:
            if day < 23 { sign = "♎️ - Libra" } else { sign = "♏️ - Escorpião" }
        case .novembro:
            if day < 22 { sign = "♏️ - Escorpião" } else { sign = "♐️ - Sagitário" }
        }
        return sign
    }
}
